...
def main():
    ...
    ok = (
        "error" not in current_result
        and not current_result.get("missing")
        and bool(current_result.get("parquet_ok"))
    )
    ...
    if good:
        ...
        for python_exe in good:
            print(f"  \"{python_exe}\" \"{SCRIPT_DIR / 'run_oa_fault_discovery.py'}\" --input <data> --cpu")
        return
